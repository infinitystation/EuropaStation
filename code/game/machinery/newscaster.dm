...
/datum/feed_network/proc/alert_readers(var/annoncement)
	...
	spawn(0)	// get_receptions sleeps further down the line, spawn of elsewhere
		...
		for(var/obj/item/radio/headset/pda/PDA in receiving_pdas)
...
/obj/machinery/newscaster
	name = "newscaster"
	desc = "A standard newsfeed handler for use on commercial spacecraft. All the news you absolutely have no use for, in one place!"
...
/obj/machinery/newscaster/attack_ai(var/mob/user)
	return src.attack_hand(user)

/obj/machinery/newscaster/attack_hand(var/mob/user)            //########### THE MAIN BEEF IS HERE! And in the proc below this...############
		...
		switch(screen)
			...
			if(9)
				...
				if(src.viewing_channel.censored)
					dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the crew, and marked with a [using_map.company_name] D-Notice.<BR>"
			...
			if(11)
				...
				dat+="<FONT SIZE=1>A D-Notice is to be bestowed upon the channel if the handling Authority deems it as harmful for the crew's"
			...
			if(13)
				dat+="<B>[src.viewing_channel.channel_name]: </B><FONT SIZE=1>\[ created by: <FONT COLOR='maroon'>[src.viewing_channel.author]</FONT> \]</FONT><BR>"
				dat+="Channel messages listed below. If you deem them dangerous to the crew, you can <A href='?src=\ref[src];toggle_d_notice=\ref[src.viewing_channel]'>Bestow a D-Notice upon the channel</A>.<HR>"
				if(src.viewing_channel.censored)
					dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the crew, and marked with a [using_map.company_name] D-Notice.<BR>"
			...
			if(17)
				...
				dat+="<BR><A href='?src=\ref[src];setScreen=[0]'>Return</A><BR>"
			if(18)
				dat+="<B><FONT COLOR ='maroon'>-- PRIORITY WANTED ISSUE --</B></FONT><BR><FONT SIZE=2>\[Submitted by: <FONT COLOR='green'>[news_network.wanted_issue.backup_author]</FONT>\]</FONT><HR>"
...
/obj/machinery/newscaster/attackby(obj/item/I as obj, var/mob/user)
	if (stat & BROKEN)
...
/obj/machinery/newscaster/proc/AttachPhoto(var/mob/user)
	if(photo_data)
...
/obj/item/newspaper
	name = "newspaper"
	desc = "An issue of The Copernican, the newspaper circulating through most human facilities."
	...
	if(ishuman(user))
		...
		switch(screen)
			if(0) //Cover
				...
				dat+="<DIV ALIGN='center'><FONT SIZE=2>[using_map.company_name]-standard newspaper, for use on [using_map.company_name] Colonial Facilities</FONT></div><HR>"
			...
			if(1) // X channel pages inbetween.
				...
				if(C.censored)
					dat+="This channel was deemed dangerous to the general welfare of the crew and therefore marked with a <B><FONT COLOR='red'>D-Notice</B></FONT>. Its contents were not transferred to the newspaper at the time of printing."
				else
...
/obj/item/newspaper/attackby(var/obj/item/W, var/mob/user)
...
/obj/machinery/newscaster/proc/scan_user(var/mob/living/user)
	if(istype(user,/mob/living/carbon/human))                       //User is a human
		var/mob/living/carbon/human/human_user = user
		if(human_user.wear_id)                                      //Newscaster scans you
			if(istype(human_user.wear_id, /obj/item/radio/headset/pda) )	//autorecognition, woo!
